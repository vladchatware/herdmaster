...
public final class EvansHTMParser {

    public init() {}

    public func parse(fileAt url: URL) throws -> EvansPedigreeDocument {
        let data = try Data(contentsOf: url)
        let html = try decodeWithFallback(data: data)
        return try parse(html: html)
    }

    public func parse(html rawHTML: String) throws -> EvansPedigreeDocument {
    ...
    private func extractAnimals(from html: String) -> [EvansAnimal] {
        ...
        for (index, match) in matches.enumerated() {
            ...
            let (column, row) = gridPositionFor(index: index)
            let animal = parseAnimalCell(contents: contents, sex: sex, column: column, row: row)
    ...
    private func gridPositionFor(index: Int) -> (column: Int, row: Int) {
        var remaining = index
        var column = 0

        while true {
            let cellsAtColumn = 1 << column
            if remaining < cellsAtColumn {
                return (column, remaining)
            }
            remaining -= cellsAtColumn
            column += 1
            if column > 10 {
                return (column, remaining)
            }
